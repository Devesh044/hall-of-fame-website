...
YOUTUBE_API_KEY = 'AIzaSyAPGQnxKotOVvrAoTbmfKr7tGL3yeSqvWM'
...
def dashboard(request):
    ...
    return render(request, 'halls/dashboard.html',{'recent_halls':recent_halls})

@login_required
def HALL(request):
        halls = Hall.objects.filter(user=request.user)
        return render(request, 'halls/HALL.html',{'halls':halls})

def Community(request):
        return render(request, 'halls/Community.html')        
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if video_id:
                ...
                errors.append('Needs to be a You Tube URL')
